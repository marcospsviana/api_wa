...
""")
  ...
  PRIMARY KEY (`id_photos`)
    ...
    def cadastro_user(self, data):
        ...
        print(data['idUser'])
        __cursor.execute("""
            INSERT INTO tb_user (id,idUser,avatarUrl,name,email,categories,subcategories,description,likes,dislikes,total_votos,localization) VALUES (null, '%s', null, '%s','%s', '%s', '%s','%s', %s, %s,%s, '%s')
            """%(data['idUser'],data['name'],data['email'],data['categories'],data['subCategories'],data['description'],0,0,0,data['localization']))
        __conn.commit()
    ...
    def cadastro_photos_user(self, idUser, key_number, dados):
        ...
        if response == None or response == '' or response == []:
            ...
            __cursor.execute("""UPDATE tb_photos_portfolio photos SET photos = JSON_SET(photos, '$.%s', '{"%s"}') WHERE idUser = '%s'"""%(key_number, dados, idUser))
            __conn.commit()
    ...
    def delete_photos_user(self, idUser, key_number):
        ...
        if response == None or response == '' or response == []:
            ...
            __cursor.execute("""UPDATE tb_photos_portfolio photos SET photos = JSON_REMOVE(photos, '$.%s') WHERE idUser = '%s'"""%(key_number, idUser))
            __conn.commit()
